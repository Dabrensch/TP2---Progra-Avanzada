		...
		for (Nodo nodo : ruta) {
			if (nodo.esRobopuerto()) {
				...
				if (cofre.equals(cofreOfrecido)) {
					...
					System.out.print("\t\t\tAgarro los items del cofre " + cofreOfrecido.getId());
					System.out.println("\t\t\t\tUbicacion actual: " + robot.getUbicacion() + ", Bateria actual: "
							+ robot.getBateria());
				} else {
					...
					System.out.print("\t\t\tDejo los items en el cofre " + cofreSolicitado.getId());
					System.out.println("\t\t\t\tUbicacion actual: " + robot.getUbicacion() + ", Bateria actual: "
							+ robot.getBateria());
				}
			}
	...
	public int atenderPedidos() {
		...
		while (!ofrecidos.isEmpty()) {
			...
			if (ofrecido.getCantidad() != 0) {
				System.out.print(
						"\n\tNo se pudó completar todo el pedido del cofre " + ofrecido.getCofre().getId() + ", quedaron "
								+ ofrecido.getCantidad() + " unidades de " + ofrecido.getItem() + " sin ofrecer. ");
		...
		for (Pedido solicitado : solicitados) {
			if (solicitado.getCantidad() != 0) {
				tieneSolucion = false;
				System.out.println("Solicitud incompleta: " + solicitado);
			}
		}
		for (Pedido pedido : sinOfrecer) {
			tieneSolucion = false;
			System.out.println("Ofrecimiento incompleto: " + pedido);
		}
		if (!tieneSolucion) {
			return -1;
		} else {
